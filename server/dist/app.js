...
import { batchTestSchema, batchUpdateSchema, ccSwitchImportSchema, codexAccountImportSchema, codexAccountPatchSchema, codexModelSyncSchema, codexProxyStartSchema, codexUsageQuerySchema, customPurityTestSchema, discoverSchema, historyQuerySchema, imageGenerationSchema, poolAddRelaysSchema, poolStartSchema, poolStrategySchema, poolUsageQuerySchema, playgroundSchema, purityTestSchema, relayOrderSchema, relayCreateSchema, relayUpdateSchema, testSchema } from './validation.js';
...
const writePurityStreamEvent = (res, event) => {
    if (res.destroyed || res.writableEnded)
        return;
    res.write(`${JSON.stringify(event)}\n`);
};
const customPurityRelay = (input) => {
...
const runCodexAction = async (action) => {
    try {
        return await action();
    }
    catch (error) {
        ...
        if (nodeError.code === 'EADDRINUSE')
            throw new HttpError(409, '指定端口已被占用');
        if (typeof nodeError.message === 'string') {
            if (nodeError.message.includes('已启动') || nodeError.message.includes('未启动'))
                throw new HttpError(409, nodeError.message);
            if (nodeError.message.includes('端口') || nodeError.message.includes('账号'))
                throw new HttpError(400, nodeError.message);
        }
        throw error;
    }
...
export async function createApp(dependencies = {}) {
    ...
    const purityControllers = new Map();
    const registerPurityController = (relayId, controller) => {
        const controllers = purityControllers.get(relayId) ?? new Set();
        controllers.add(controller);
        purityControllers.set(relayId, controllers);
    };
    const unregisterPurityController = (relayId, controller) => {
        const controllers = purityControllers.get(relayId);
        if (!controllers)
            return;
        controllers.delete(controller);
        if (!controllers.size)
            purityControllers.delete(relayId);
    };
    const cancelPurityTests = (relayId) => {
    ...
    app.post('/api/relays/:id/playground/stream', express.json({ limit: '512kb' }), asyncHandler(async (req, res) => {
    ...
    app.post('/api/codex-accounts/import', express.json({ limit: '2mb' }), asyncHandler(async (req, res) => {
    ...
    app.get('/api/codex-accounts', asyncHandler(async (_req, res) => send(res, await codexAccounts.listPublic())));
    app.patch('/api/codex-accounts/:id', asyncHandler(async (req, res) => send(res, await codexAccounts.update(pathParam(req, 'id'), codexAccountPatchSchema.parse(req.body ?? {})), 'GPT 账号已更新')));
    app.delete('/api/codex-accounts/:id', asyncHandler(async (req, res) => {
    ...
    app.post('/api/codex-accounts/:id/models', asyncHandler(async (req, res) => {
        const controller = new AbortController();
    ...
    app.post('/api/codex-accounts/:id/usage', asyncHandler(async (req, res) => {
        const controller = new AbortController();
        const onClose = () => { if (!res.writableEnded)
            controller.abort(); };
        res.on('close', onClose);
    ...
    app.post('/api/codex-accounts/models', asyncHandler(async (req, res) => {
    ...
    app.post('/api/codex-proxy/start', asyncHandler(async (req, res) => {
        const result = await runCodexAction(() => codexProxy.start(codexProxyStartSchema.parse(req.body ?? {})));
        send(res, result, 'GPT 账号服务已启动');
    }));
    app.post('/api/codex-proxy/stop', asyncHandler(async (_req, res) => send(res, await runCodexAction(() => codexProxy.stop()), 'GPT 账号服务已停止')));
    app.post('/api/codex-proxy/key/rotate', asyncHandler(async (_req, res) => send(res, await runCodexAction(() => codexProxy.rotateKey()), 'GPT 账号服务 API Key 已轮换')));
    app.get('/api/codex-proxy/usage', asyncHandler(async (req, res) => send(res, await codexUsage.report(codexUsageQuerySchema.parse(req.query)))));
    app.delete('/api/codex-proxy/usage', asyncHandler(async (_req, res) => {
    ...
    app.post('/api/pool/relays', asyncHandler(async (req, res) => {
    ...
    app.get('/api/relays/export', asyncHandler(async (_req, res) => {
    ...
    app.post('/api/relays/import', express.raw({
        type: [
        ...
        if (!Buffer.isBuffer(req.body) || !req.body.length)
            throw new HttpError(400, '请上传 Excel 文件');
    ...
    app.post('/api/purity-test', asyncHandler(async (req, res) => {
        ...
        res.on('close', onClose);
        try {
            send(res, await purityTester.test(relay, {
                model: input.model,
                mode: input.mode,
                gpt56: input.gpt56,
                signal: controller.signal
    ...
    app.post('/api/purity-test/stream', asyncHandler(async (req, res) => {
        const input = customPurityTestSchema.parse(req.body ?? {});
        const relay = customPurityRelay(input);
        const controller = new AbortController();
        const onClose = () => {
    ...
    app.post('/api/relays/:id/purity-test', asyncHandler(async (req, res) => {
        const input = purityTestSchema.parse(req.body ?? {});
        const relayId = pathParam(req, 'id');
        const relay = await relays.find(relayId);
        if (!relay.enabled)
            throw new HttpError(409, '已停用的中转站不能执行纯度检测');
        const controller = new AbortController();
        const onClose = () => {
            if (!res.writableEnded)
                controller.abort();
        ...
        try {
            send(res, await purityTester.test(relay, { ...input, signal: controller.signal }), '纯度检测完成');
        }
        catch (error) {
            if (error instanceof Error && error.name === 'AbortError') {
                throw new HttpError(499, '纯度检测已取消');
    ...
    app.post('/api/relays/:id/purity-test/stream', asyncHandler(async (req, res) => {
        const input = purityTestSchema.parse(req.body ?? {});
        const relayId = pathParam(req, 'id');
        const relay = await relays.find(relayId);
        if (!relay.enabled)
