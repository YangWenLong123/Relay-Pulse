    ...
    if (endOffset >= 0 && endOffset + 22 <= buffer.length) {
        ...
        for (let index = 0; index < entryCount && offset + 46 <= buffer.length; index += 1) {
            ...
            if (method === 0)
                ...
                throw new Error('Excel 文件使用了不支持的压缩方式');
...
function sheetTarget(entries) {
    const workbook = entries.get('xl/workbook.xml');
    if (!workbook)
        throw new Error('Excel 文件缺少工作簿');
...
export function parseXlsxRows(buffer) {
    ...
    const shared = [...sharedStrings.matchAll(/<si>([\s\S]*?)<\/si>/g)].map((match) => [...(match[1] ?? '').matchAll(/<t(?:\s[^>]*)?>([\s\S]*?)<\/t>/g)].map((item) => xmlDecode(item[1] ?? '')).join(''));
    const rows = [];
    for (const rowMatch of sheet.toString('utf8').matchAll(/<row\b[^>]*>([\s\S]*?)<\/row>/g)) {
        ...
        for (const cellMatch of (rowMatch[1] ?? '').matchAll(/<c\b([^>]*)>([\s\S]*?)<\/c>|<c\b([^>]*)\/>/g)) {
            ...
            if (type === 'inlineStr')
                row[column] = [...content.matchAll(/<t(?:\s[^>]*)?>([\s\S]*?)<\/t>/g)].map((item) => xmlDecode(item[1] ?? '')).join('');
            else if (type === 's')
                row[column] = shared[Number(value)] ?? '';
            else if (type === 'b')
                row[column] = value === '1';
            else if (value === '')
                row[column] = '';
            else
                row[column] = Number.isFinite(Number(value)) ? Number(value) : xmlDecode(value);
        }
