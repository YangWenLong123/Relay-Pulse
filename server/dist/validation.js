...
export const imageGenerationSchema = z.object({
    ...
    if (input.source === 'custom') {
        if (!input.baseUrl)
            ctx.addIssue({ code: z.ZodIssueCode.custom, path: ['baseUrl'], message: '请输入 Base URL' });
        if (!input.apiKey)
            ctx.addIssue({ code: z.ZodIssueCode.custom, path: ['apiKey'], message: '请输入 API Key' });
    }
});
...
export const poolStartSchema = z
    ...
    modelMap: z
        .record(z.string().uuid(), z.array(z.string().trim().min(1).max(160)).max(500)
