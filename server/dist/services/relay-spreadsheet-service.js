...
import { buildXlsx, parseXlsxRows } from '../lib/xlsx.js';
const columns = [
    { key: 'id', label: 'ID' },
    { key: 'name', label: '名称' },
...
function valueAt(relay, key) {
    const [scope, field] = key.split('.', 2);
    if (!field)
        return relay[scope];
    const source = relay[scope];
    return source?.[field];
}
function text(value) {
...
function booleanValue(value, fallback) {
    ...
    if (['true', '1', 'yes', 'y', '是', '启用', '成功'].includes(normalized))
        return true;
    if (['false', '0', 'no', 'n', '否', '停用', '失败'].includes(normalized))
        return false;
    return fallback;
}
...
function balanceConfigFromRow(row, indexes) {
    ...
    if (!keys.some((key) => text(rowValue(row, indexes, key))))
        return undefined;
...
export function importRelays(buffer) {
    ...
    for (const required of ['name', 'baseUrl', 'apiKey', 'model']) {
        if (!headerIndexes.has(required))
            throw new HttpError(400, `Excel 缺少必要列：${columns.find((column) => column.key === required)?.label ?? required}`);
    }
    const inputs = [];
    rows.slice(1).forEach((row, index) => {
        ...
        if (!name || !baseUrl || !apiKey || !model)
            throw new HttpError(400, `第 ${rowNumber} 行缺少名称、Base URL、API Key 或模型`);
        ...
        const protocol = ['auto', 'responses', 'chat'].includes(text(rowValue(row, headerIndexes, 'protocol')))
            ? text(rowValue(row, headerIndexes, 'protocol'))
            : 'auto';
        inputs.push({
            name,
            baseUrl,
    ...
    if (!inputs.length)
        throw new HttpError(400, 'Excel 中没有可导入的中转站');
    if (inputs.length > 500)
        throw new HttpError(400, '一次最多导入 500 个中转站');
    return inputs;
}
