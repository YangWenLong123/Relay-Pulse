...
function compactError(status, body) {
...
function parseUsageWindow(headers, prefix) {
...
function usageSnapshotFromHeaders(headers, fallbackPlanType) {
    ...
    if (!primary && !secondary && !activeLimit && !headers.has('x-codex-plan-type'))
        return null;
    return {
        planType,
        activeLimit,
...
function sessionWithToken(session, token) {
    const next = {
        ...session,
...
function modelsEndpoint(baseUrl, clientVersion) {
...
export class CodexAccountService {
    ...
    async discoverModels(id, signal) {
    ...
    async discoverMany(ids, signal) {
        const results = await Promise.all(ids.map(async (id) => {
            try {
                return { ok: true, account: (await this.discoverModels(id, signal)).account };
            }
            catch (error) {
        ...
        return {
            accounts: results.filter((result) => result.ok).map((result) => result.account),
            failed: results.filter((result) => !result.ok)
                .map(({ id, message }) => ({ id, message }))
        };
    }
    async refreshUsage(id, signal) {
    ...
    async fetchModels(account, signal) {
        ...
        try {
            ...
            if (error.name === 'AbortError')
                throw new HttpError(504, '模型探测超时');
    ...
    async fetchUsage(account, signal) {
        ...
        try {
            ...
            if (error.name === 'AbortError')
                throw new HttpError(504, '额度刷新超时');
    ...
    async refreshSession(session, signal) {
        if (!session.refresh_token)
            throw new HttpError(409, '账号缺少 refresh_token，请重新导入 session');
        const controller = new AbortController();
        const onAbort = () => controller.abort();
        signal?.addEventListener('abort', onAbort, { once: true });
        const timer = setTimeout(() => controller.abort(), this.timeoutMs);
        try {
            ...
            const body = await response.text();
            if (!response.ok)
                throw new HttpError(response.status >= 400 && response.status < 500 ? response.status : 502, compactError(response.status, body));
            const parsed = body ? JSON.parse(body) : null;
            ...
            if (error instanceof HttpError)
                throw error;
            if (error.name === 'AbortError')
                throw new HttpError(504, '刷新账号凭证超时');
