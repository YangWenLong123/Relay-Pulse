...
function genericSnapshot(payload) {
    ...
    if (remaining === null)
        throw new Error('返回内容不包含可识别的余额字段');
    return {
        remaining,
        total,
        used,
...
function newApiSnapshot(payload) {
    if (!isRecord(payload))
        throw new Error('余额接口返回格式无效');
    if (payload.success !== true || !isRecord(payload.data))
        throw new Error(text(payload.message, '查询失败'));
    ...
    if (quota === null || usedQuota === null)
        throw new Error('返回内容不包含可识别的额度字段');
    return {
...
function requestUrl(relay) {
    ...
    if (parsed.pathname.replace(/\/+$/, '').endsWith(endpoint))
        ...
        try {
            ...
            if (config.template === 'newapi') {
                if (!config.accessToken || !config.userId)
                    throw new Error('请填写 New API 访问令牌和用户 ID');
            ...
            if (!response.ok)
                throw new Error(response.status === 401 || response.status === 403 ? '凭证无效或没有查询权限' : `查询请求失败（HTTP ${response.status}）`);
            let payload;
            try {
            ...
            return {
                ...
                unit: '',
                planName: '',
                errorMessage: controller.signal.aborted ? '余额查询超时或已取消' : error.message || '余额查询失败',
                queriedAt
            };
        }
        finally {
