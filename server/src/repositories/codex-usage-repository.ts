...
function updateSummary(summary: PoolUsageSummary, record: CodexUsageRecord): void {
...
function breakdown(
  ...
  keyOf: (record: CodexUsageRecord) => string,
  labelOf: (record: CodexUsageRecord) => string
): PoolUsageBreakdown[] {
...
function filterOptions(records: CodexUsageRecord[]): CodexUsageFilterOptions {
  const models = new Map<string, { value: string; label: string }>();
  const accounts = new Map<string, { value: string; label: string }>();
  records.forEach((record) => {
    if (record.model) models.set(record.model, { value: record.model, label: record.model });
    if (record.accountId) accounts.set(record.accountId, { value: record.accountId, label: record.accountLabel || '未命名账号' });
  });
  const sort = (items: Iterable<{ value: string; label: string }>) =>
    [...items].sort((left, right) => left.label.localeCompare(right.label) || left.value.localeCompare(right.value));
  return { models: sort(models.values()), accounts: sort(accounts.values()) };
}

export class CodexUsageRepository {
  private readonly store: JsonStore<CodexUsageRecord[]>;
  ...
  async add(record: CodexUsageRecord): Promise<void> {
  ...
  async report(query: CodexUsageQuery): Promise<CodexUsageReport> {
    ...
    const records = contextual.filter((record) =>
      (!query.model || record.model === query.model) && (!query.accountId || record.accountId === query.accountId)
    );
