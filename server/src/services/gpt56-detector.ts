...
import type {
  ...
  Gpt56Effort,
  Gpt56EncryptedStateSummary,
  Gpt56JuiceObservation,
  Gpt56JuiceSummary,
  Gpt56LiteralControlSummary,
  Gpt56NetworkSummary,
  Gpt56ProgressStage,
  Gpt56TestConfig,
  PurityCheckResult,
...
type OutputKind = 'number' | 'refusal' | 'other';
...
interface DetectorSession {
  ...
  model: string;
  trials: number;
  trustedReference?: Gpt56TestConfig['trustedReference'];
  startedAt: number;
  /** Candidate-only measurements are the only metrics exposed in the test result. */
  candidateMetrics: RequestMetrics;
  /** Trusted-reference traffic is deliberately isolated from the candidate report. */
  trustedMetrics: RequestMetrics;
  encrypted: Gpt56EncryptedStateSummary;
  juiceObservations: Gpt56JuiceObservation[];
  literal: Gpt56LiteralControlSummary;
}

export interface Gpt56DetectorOptions {
  relay: Relay;
  model: string;
  config?: Gpt56TestConfig;
  signal?: AbortSignal;
...
class ResponsesRequestError extends Error {
  constructor(
    message: string,
    readonly category: 'auth' | 'rate_limit' | 'incompatible' | 'upstream' | 'timeout' | 'transport' | 'invalid_response' | 'http',
...
const EFFORTS: Gpt56Effort[] = ['low', 'medium', 'high', 'xhigh', 'max'];
const POSITIVE_VARIANTS: CandidateVariant[] = ['full', 'without_ids'];
const NEGATIVE_VARIANTS: CandidateVariant[] = ['message_only', 'corrupted_ciphertext'];
const OUTPUT_LITERAL_VALUES = ['48', '32'] as const;
const REFUSAL_MARKERS = ['can\'t provide', 'cannot provide', 'can’t provide', 'unable to provide', '不能提供', '无法提供', '不能透露', '无法透露'];
const DISCLAIMER =
  '加密状态结果仅证明本次请求观察到的 Responses 兼容能力，Juice 是辅助可见输出指纹；两者均不能证明物理上游、模型权重或其他业务请求的真实来源。';
...
const ENCRYPTED_TASKS = [
  {
    ...
    transform: (value: string): string => value.split('').reverse().join('')
  },
  {
    id: 'rotate_left_3',
    ...
    transform: (value: string): string => `${value.slice(3)}${value.slice(0, 3)}`
  },
  {
    id: 'complement_9',
    ...
    transform: (value: string): string => value.split('').map((digit) => `${9 - Number(digit)}`).join('')
  }
] as const;
const RECALL_PROMPT =
  'Return the exact ten-digit value B computed and retained in the preceding hidden reasoning state. Output only B. If that state is absent or unreadable, output exactly UNKNOWN. Do not recompute, guess, or create a replacement value.';
...
function responseOutput(payload: JsonRecord): JsonRecord[] | null {
...
function sanitizeErrorBody(raw: string): string {
  try {
    ...
    if (isRecord(parsed)) {
      ...
      if (typeof error.message === 'string') return error.message.replace(/Bearer\s+[^\s"']+/gi, 'Bearer ***').slice(0, 240);
    }
  } catch {
...
function upstreamError(status: number, raw: string, durationMs: number): ResponsesRequestError {
  const detail = sanitizeErrorBody(raw);
  if (status === 401 || status === 403) return new ResponsesRequestError('API Key 无效或没有访问权限', 'auth', status, durationMs);
  if (status === 429) return new ResponsesRequestError('上游请求频率受限', 'rate_limit', status, durationMs);
  ...
  if (status >= 500) return new ResponsesRequestError('上游服务暂时不可用', 'upstream', status, durationMs);
...
class ResponsesClient {
  ...
  async post(payload: JsonRecord, signal?: AbortSignal): Promise<ResponsesResponse> {
    ...
    try {
      ...
      if (!response.ok) throw upstreamError(response.status, raw, durationMs);
      let parsed: unknown;
      try {
...
function addUsage(metrics: RequestMetrics, payload: JsonRecord): void {
  const usage = isRecord(payload.usage) ? payload.usage : undefined;
  const add = (field: 'inputTokens' | 'outputTokens' | 'totalTokens', value: number | null): void => {
  ...
  if (typeof payload.model === 'string' && payload.model.trim()) metrics.reportedModels.add(payload.model.trim().slice(0, 160));
}

function isRetryable(error: ResponsesRequestError): boolean {
...
async function postWithRetry(
  ...
  let retryCount = 0;
  let error: ResponsesRequestError | undefined;
  for (let attempt = 1; attempt <= MAX_TRANSPORT_ATTEMPTS; attempt += 1) {
    ...
    try {
      ...
      if (attempt >= MAX_TRANSPORT_ATTEMPTS || !isRetryable(error)) break;
      retryCount += 1;
...
function encryptedSeedPayload(model: string, prompt: string): JsonRecord {
  return {
    model,
...
function encryptedRecallPayload(model: string, context: JsonRecord[]): JsonRecord {
  return {
    model,
...
function encryptedReasoningCount(output: JsonRecord[]): number {
...
function removeEncryptedContent(value: unknown): unknown {
  if (Array.isArray(value)) return value.map(removeEncryptedContent);
  if (!isRecord(value)) return value;
  return Object.fromEntries(
    Object.entries(value).map(([key, child]) => [key, key === 'encrypted_content' ? '[REDACTED]' : removeEncryptedContent(child)])
  );
}

function hasVisibleLeak(value: unknown, secrets: string[]): boolean {
...
function contextVariant(output: JsonRecord[], variant: CandidateVariant): JsonRecord[] {
  if (variant === 'full') return cloneJson(output);
  if (variant === 'message_only') return cloneJson(output.filter((item) => item.type === 'message'));
  ...
  for (const item of context) {
    if (item.type !== 'reasoning' || typeof item.encrypted_content !== 'string' || !item.encrypted_content) continue;
...
function normalizeNumber(value: string): string | null {
  ...
  if (normalized.startsWith('```') && normalized.endsWith('```')) {
    ...
    if (lines.length >= 3) normalized = lines.slice(1, -1).join('\n').trim();
  }
...
function classifyOutput(value: string): { kind: OutputKind; normalizedValue: string | null } {
  const normalizedValue = normalizeNumber(value);
  if (normalizedValue !== null) return { kind: 'number', normalizedValue };
...
function solSignatureMatches(effort: Gpt56Effort, value: string): boolean {
  if (effort === 'low') return value === '8' || /^8\.\d+$/.test(value) || /^8\d{2,}$/.test(value);
  if (effort === 'medium') return value === '16' || /^16\.\d+$/.test(value) || /^16\d{2,}$/.test(value);
  if (effort === 'high') return value === '40' || /^40(?:\.\d+|\d{2,})$/.test(value);
  if (effort === 'xhigh') return value === '128';
  return value === '960';
}

const EXACT_SIGNATURES: Record<string, Partial<Record<Gpt56Effort, string>>> = {
...
function matchingModels(effort: Gpt56Effort, value: string | null): string[] {
  if (value === null) return [];
  const matches = solSignatureMatches(effort, value) ? ['gpt_5_6_sol'] : [];
  for (const [model, signature] of Object.entries(EXACT_SIGNATURES)) {
    if (signature[effort] === value) matches.push(model);
  }
  return matches;
}

function highGroup(models: string[]): string | null {
  if (models.includes('gpt_5_6_sol')) return 'gpt_5_6_sol';
  if (models.includes('gpt_5_6_terra')) return 'gpt_5_6_terra';
  if (models.includes('gpt_5_6_luna')) return 'gpt_5_6_luna';
  if (models.includes('gpt_5_4_mini')) return 'gpt_5_4_mini';
  if (models.includes('gpt_5_5') || models.includes('gpt_5_4')) return 'gpt_5_5_or_5_4';
  return null;
}

function matchesGroup(group: string, models: string[]): boolean {
  if (group === 'gpt_5_5_or_5_4') return models.includes('gpt_5_5') || models.includes('gpt_5_4');
...
function requestedModelGroup(model: string): string | null {
  const normalized = model.trim().toLowerCase().replace(/[_.]/g, '-');
  if (/gpt-?5(?:[.-]?6)?-?sol\b/.test(normalized)) return 'gpt_5_6_sol';
  if (/gpt-?5(?:[.-]?6)?-?terra\b/.test(normalized)) return 'gpt_5_6_terra';
  if (/gpt-?5(?:[.-]?6)?-?luna\b/.test(normalized)) return 'gpt_5_6_luna';
  if (/gpt-?5(?:[.-]?4)?-?mini\b/.test(normalized)) return 'gpt_5_4_mini';
  if (/gpt-?5[.-]?5\b/.test(normalized) || /gpt-?5[.-]?4\b/.test(normalized)) return 'gpt_5_5_or_5_4';
  return null;
}

function emptyJuiceSummary(status: Gpt56JuiceSummary['status'] = 'not_run'): Gpt56JuiceSummary {
  return {
    status,
    likelyModel: null,
    confidence: 'none',
...
function summarizeJuice(observations: Gpt56JuiceObservation[], running = false): Gpt56JuiceSummary {
  ...
  if (!observations.length) return emptyJuiceSummary(running ? 'running' : 'not_run');
...
function networkSummary(metrics: RequestMetrics): Gpt56NetworkSummary {
...
function encryptedSummary(session: DetectorSession, running = false): Gpt56EncryptedStateSummary {
  const source = session.encrypted;
  if (!source.enabled) return { ...source };
  const completed = source.attempts >= source.targetAttempts;
  let status: Gpt56EncryptedStateSummary['status'] = running || !completed ? 'running' : 'inconclusive';
  let summary = running
...
function literalSummary(literal: Gpt56LiteralControlSummary, running = false): Gpt56LiteralControlSummary {
...
function detectionSnapshot(session: DetectorSession, stage: Gpt56ProgressStage, running = true): Gpt56DetectionSummary {
  return {
    stage,
...
function check(id: string, name: string, status: PurityCheckStatus, score: number | null, summary: string, requestCount: number, durationMs: number): PurityCheckResult {
  return { id, name, status, score, weight: 0, summary, evidence: [], requestCount, durationMs };
}

function statusForEncrypted(summary: Gpt56EncryptedStateSummary): PurityCheckStatus {
  if (!summary.enabled) return 'skipped';
  if (summary.status === 'compatible' || summary.status === 'preliminary_compatible') return 'pass';
  if (summary.status === 'invalid' || summary.status === 'suspicious') return 'fail';
  return 'warning';
}

function statusForJuice(summary: Gpt56JuiceSummary): PurityCheckStatus {
  if (summary.status === 'fingerprint') return 'pass';
  if (summary.status === 'mixed') return 'fail';
  return 'warning';
}

function statusForLiteral(summary: Gpt56LiteralControlSummary): PurityCheckStatus {
  if (summary.status === 'passed') return 'pass';
  if (summary.status === 'output_rewrite_suspected') return 'warning';
  return 'warning';
}

function deepChecks(snapshot: Gpt56DetectionSummary, elapsedMs: number): PurityCheckResult[] {
  const checks: PurityCheckResult[] = [];
  checks.push(check(
    'gpt56_encrypted_state',
    '加密推理状态兼容性',
    statusForEncrypted(snapshot.encrypted),
    snapshot.encrypted.status === 'compatible' ? 100 : snapshot.encrypted.status === 'preliminary_compatible' ? 80 : null,
    snapshot.encrypted.summary,
    snapshot.encrypted.attempts * 4,
    elapsedMs
  ));
  checks.push(check(
    'gpt56_juice_fingerprint',
    'Juice 可见输出指纹',
    statusForJuice(snapshot.juice),
    snapshot.juice.status === 'fingerprint' ? 100 : snapshot.juice.status === 'mixed' ? 0 : null,
    snapshot.juice.summary,
    snapshot.juice.observations.length,
    elapsedMs
  ));
  checks.push(check(
    'gpt56_literal_control',
    '高档字面量输出完整性',
    statusForLiteral(snapshot.literalControl),
    snapshot.literalControl.status === 'passed' ? 100 : null,
    snapshot.literalControl.summary,
    snapshot.literalControl.completed,
    elapsedMs
  ));
  checks.push(check(
    'gpt56_network',
    '检测线路质量',
    snapshot.network.status === 'smooth' ? 'pass' : snapshot.network.status === 'intermittent' ? 'warning' : 'fail',
    snapshot.network.status === 'smooth' ? 100 : snapshot.network.status === 'intermittent' ? 70 : 25,
...
function emitProgress(session: DetectorSession, stage: Gpt56ProgressStage, message: string, onProgress?: (progress: PurityTestProgress) => void): void {
...
async function recall(
  ...
  if (hasVisibleLeak(payload, [expected])) return { exact: false, error: false, plaintextLeak: true };
  const outcome = await postWithRetry(client, payload, metrics, signal);
  if (!outcome.response) return { exact: false, error: true, plaintextLeak: false };
  return { exact: outcome.response.text === expected, error: false, plaintextLeak: false };
}

async function runEncryptedRound(session: DetectorSession, index: number, signal?: AbortSignal, onProgress?: (progress: PurityTestProgress) => void): Promise<void> {
  ...
  const seed = await postWithRetry(
    trustedClient,
    encryptedSeedPayload(reference.model, task.prompt.replace('{value}', input)),
    session.trustedMetrics,
    signal
  );
  if (!seed.response) {
  ...
  const selfCheck = await recall(
    trustedClient,
    reference.model,
    contextVariant(output, 'full'),
    expected,
    session.trustedMetrics,
    signal
  );
  ...
  for (const variant of shuffle([...POSITIVE_VARIANTS, ...NEGATIVE_VARIANTS])) {
    ...
    if (outcome.error) session.encrypted.candidateErrors += 1;
    if (outcome.plaintextLeak) session.encrypted.plaintextLeaks += 1;
    if (variant === 'full' && outcome.exact) session.encrypted.fullExact += 1;
    if (variant === 'without_ids' && outcome.exact) session.encrypted.withoutIdsExact += 1;
    if (variant === 'message_only' && outcome.exact) session.encrypted.messageOnlyExact += 1;
    if (variant === 'corrupted_ciphertext' && outcome.exact) session.encrypted.corruptedCiphertextExact += 1;
    emitProgress(session, 'encrypted_controls', `第 ${index + 1} 轮 ${variant} 对照已返回。`, onProgress);
  }
}

function juicePayload(model: string, effort: Gpt56Effort): JsonRecord {
  const template = JUICE_PROMPTS[randomInt(0, JUICE_PROMPTS.length)] as string;
...
async function runJuiceObservation(session: DetectorSession, effort: Gpt56Effort, signal?: AbortSignal): Promise<void> {
...
function literalPayload(model: string, expected: string): JsonRecord {
  const template = LITERAL_PROMPTS[randomInt(0, LITERAL_PROMPTS.length)] as string;
...
async function runLiteralControl(session: DetectorSession, expected: string, signal?: AbortSignal): Promise<void> {
  const client = new ResponsesClient(session.relay.baseUrl, session.relay.apiKey, session.relay.timeout);
  ...
  if (outcome.response.text === expected) session.literal.exact += 1;
  else session.literal.nonExact += 1;
}

function makeSession(options: Gpt56DetectorOptions): DetectorSession {
...
function resultVerdict(snapshot: Gpt56DetectionSummary, model: string): { verdict: PurityVerdict; confidence: PurityConfidence; summary: string; anomalies: string[] } {
...
function unsupportedResult(
  options: Gpt56DetectorOptions,
  summary = 'GPT-5.6 深度检测仅支持 OpenAI Responses 协议，请选择 OpenAI 中转站并使用 Responses。'
): PurityTestResult {
...
function responsesPreflightPayload(model: string): JsonRecord {
  return {
    model,
...
function preflightFailureSummary(error?: ResponsesRequestError): string {
...
function preflightFailureResult(session: DetectorSession, summary: string): PurityTestResult {
  const elapsedMs = Math.round(performance.now() - session.startedAt);
  const snapshot = detectionSnapshot(session, 'finalizing', false);
  return {
    id: randomUUID(),
    ...
    protocol: 'responses' as TestProtocol,
    mode: 'gpt56',
 ...
 */
export async function runGpt56Detection(options: Gpt56DetectorOptions): Promise<PurityTestResult> {
  if (options.relay.platform !== 'openai' || options.relay.protocol === 'chat') return unsupportedResult(options);
  ...
  for (let index = 0; index < jobs.length; index += 1) {
    const effort = jobs[index] as Gpt56Effort;
