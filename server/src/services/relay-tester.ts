...
export class RelayTester {
  ...
  async discoverModels(
    ...
    try {
      ...
      throw makeFailure(sanitizeSecret((error as Error).message || '模型探测失败', relay.apiKey), classifyNetworkError(error));
    } finally {
  ...
  private result(
    ...
    result: Pick<
      TestResult,
      'success' | 'statusCode' | 'responseText' | 'firstByteDuration' | 'errorType' | 'errorMessage'
    >
  ): TestResult {
    return {
      id: randomUUID(),
