...
export type ImageGenerationFetch = (input: RequestInfo | URL, init?: RequestInit) => Promise<Response>;
...
const OUTPUT_FORMAT_MIME_TYPES: Record<ImageOutputFormat, string> = {
  jpg: 'image/jpeg',
  png: 'image/png',
  webp: 'image/webp'
};

const ASPECT_RATIO_LABELS: Record<ImageGenerationInput['aspectRatio'], string> = {
  '1:1': '正方形 1:1',
  '16:9': '横向宽屏 16:9',
  '9:16': '竖向海报 9:16',
  '4:3': '横向标准 4:3',
...
function decodeReferenceImage(reference: ImageReferenceInput): DecodedReference {
  const match = /^data:([^;,]+);base64,(.+)$/i.exec(reference.dataUrl.trim());
  if (!match) throw new HttpError(400, '参考图格式无效，请上传 PNG、JPG 或 WebP 图片');
  const mimeType = (reference.mimeType || match[1] || '').toLowerCase();
  if (!ALLOWED_REFERENCE_MIME_TYPES.has(mimeType)) throw new HttpError(400, '参考图仅支持 PNG、JPG 或 WebP');
  const buffer = Buffer.from(match[2]!, 'base64');
  if (!buffer.length) throw new HttpError(400, '参考图数据为空');
  if (buffer.length > MAX_REFERENCE_IMAGE_BYTES) throw new HttpError(413, '参考图不能超过 8MB');
...
function nextImageRequestOptions(status: number, raw: string, options: ImageRequestOptions): ImageRequestOptions | null {
  if (status !== 400 || !/response_format|output_format|unknown parameter|unsupported parameter|unrecognized|invalid.*parameter/i.test(raw)) return null;
  ...
  if (options.includeOutputFormat) return { ...options, includeOutputFormat: false };
  return null;
}

function mimeTypeFromBase64(value: string, fallback: string): string {
...
function parseImagePayload(payload: unknown, format: ImageOutputFormat): Omit<ImageGenerationResultImage, 'index'>[] {
  ...
  const images: Omit<ImageGenerationResultImage, 'index'>[] = [];
  for (const item of payload.data.filter(isRecord)) {
    ...
    if (b64Json) {
      ...
      const image: Omit<ImageGenerationResultImage, 'index'> = { dataUrl: `data:${mimeType};base64,${b64Json}`, mimeType };
      if (revisedPrompt) image.revisedPrompt = revisedPrompt;
      images.push(image);
      continue;
    }
    if (url) {
      const image: Omit<ImageGenerationResultImage, 'index'> = { url, mimeType: fallbackMimeType };
      if (revisedPrompt) image.revisedPrompt = revisedPrompt;
      images.push(image);
    }
  }
...
function imageEndpoint(hasReference: boolean): '/v1/images/generations' | '/v1/images/edits' {
  return hasReference ? '/v1/images/edits' : '/v1/images/generations';
}

export class ImageGenerationService {
  private readonly fetcher: ImageGenerationFetch;
  ...
  constructor(options: ImageGenerationServiceOptions = {}) {
    ...
    if (!Number.isSafeInteger(this.timeoutMs) || this.timeoutMs <= 0) throw new Error('timeoutMs 必须是正整数');
  }

  async generate(relay: Relay, input: ImageGenerationInput, signal?: AbortSignal): Promise<ImageGenerationResult> {
    if (!relay.enabled) throw new HttpError(400, '请选择已启用的中转站');
    if (relay.platform !== 'openai') throw new HttpError(400, '绘图功能暂仅支持 OpenAI 兼容中转站');
    ...
    const images: ImageGenerationResultImage[] = [];
    ...
    if (!firstImage) throw new HttpError(502, '上游绘图接口未返回可用图片');
  ...
  private async requestImage(
    ...
    reference: DecodedReference | undefined,
    signal?: AbortSignal
  ): Promise<unknown> {
    let firstRaw = '';
    let options: ImageRequestOptions = { includeResponseFormat: true, includeOutputFormat: true };
  ...
  private async fetchUpstream(
    ...
    try {
      ...
      if (controller.signal.aborted) {
        if (timedOut) throw new HttpError(504, '绘图请求超时，请稍后重试或更换中转站');
  ...
  private generationBody(model: string, size: ImageSize, prompt: string, format: ImageOutputFormat, options: ImageRequestOptions): Record<string, unknown> {
    return {
      model,
      prompt,
      size,
  ...
  private editFormData(
    ...
    form.set('size', size);
    form.set('n', '1');
    if (options.includeResponseFormat) form.set('response_format', 'b64_json');
    if (options.includeOutputFormat) form.set('output_format', format);
