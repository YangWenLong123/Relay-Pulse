...
export type PlaygroundFetch = typeof fetch;

export interface PlaygroundServiceOptions {
  fetch?: PlaygroundFetch;
}

export interface PlaygroundGenerateOptions {
  signal?: AbortSignal;
...
function usageFrom(payload: unknown): PlaygroundUsage {
  if (!isRecord(payload)) return { inputTokens: null, outputTokens: null, totalTokens: null };
...
function mergeUsage(current: PlaygroundUsage, next: PlaygroundUsage): PlaygroundUsage {
...
function compactUpstreamError(raw: string, relay: Relay): string {
  let message = raw;
  try {
...
function providerError(payload: JsonRecord): string | null {
  ...
  if (payload.type === 'error' && typeof payload.message === 'string') return payload.message.trim();
  if (payload.type === 'response.failed') return '上游模型生成失败';
  return null;
}

function reportedModel(payload: JsonRecord): string | null {
  if (typeof payload.model === 'string') return payload.model;
  if (isRecord(payload.response) && typeof payload.response.model === 'string') return payload.response.model;
  if (isRecord(payload.message) && typeof payload.message.model === 'string') return payload.message.model;
  return null;
}

function finishReason(payload: JsonRecord): string | null {
  if (Array.isArray(payload.choices)) {
    const choice = payload.choices.find(isRecord);
    if (typeof choice?.finish_reason === 'string') return choice.finish_reason;
  }
  if (isRecord(payload.delta) && typeof payload.delta.stop_reason === 'string') return payload.delta.stop_reason;
  if (isRecord(payload.response)) {
    if (typeof payload.response.status === 'string' && payload.response.status !== 'in_progress') return payload.response.status;
...
function deltaText(payload: JsonRecord, protocol: TestProtocol): string {
  if (protocol === 'chat') {
...
function requestBody(input: PlaygroundInput, protocol: TestProtocol): JsonRecord {
  const sampling = {
...
function protocolEndpoint(protocol: TestProtocol): string {
  if (protocol === 'responses') return '/v1/responses';
  if (protocol === 'anthropic') return '/v1/messages';
  return '/v1/chat/completions';
}

function protocolHeaders(relay: Relay, protocol: TestProtocol): Record<string, string> {
  if (protocol === 'anthropic') {
    return {
...
export class PlaygroundService {
  private readonly fetcher: PlaygroundFetch;
  ...
  async generate(
    ...
    options: PlaygroundGenerateOptions = {}
  ): Promise<PlaygroundCompletion> {
    if (!relay.enabled) throw new HttpError(409, '已停用的中转站不能用于游乐场');
    const protocols: TestProtocol[] = relay.platform === 'anthropic'
      ? ['anthropic']
      : relay.protocol === 'auto'
        ? ['responses', 'chat']
        : [relay.protocol];
  ...
  private async attempt(
    ...
    try {
      ...
      if (!state.outputCharacters) throw new HttpError(502, '上游返回成功，但模型回复为空');
      return {
        relayId: relay.id,
      ...
      const message = compactUpstreamError((error as Error).message || '连接上游失败', relay);
  ...
  private emitDelta(state: StreamState, text: string, onDelta?: (text: string) => void): void {
    if (!text) return;
    state.outputCharacters += text.length;
    if (state.outputCharacters > MAX_OUTPUT_CHARACTERS) throw new HttpError(502, '模型回复超过游乐场长度限制');
    onDelta?.(text);
  }

  private applyPayload(payload: JsonRecord, protocol: TestProtocol, state: StreamState, onDelta?: (text: string) => void): void {
    const error = providerError(payload);
    if (error) throw new HttpError(502, error.slice(0, MAX_UPSTREAM_ERROR_LENGTH));
  ...
  private async consumeEventStream(
    ...
    if (!response.body) throw new HttpError(502, '上游流式响应没有可读取内容');
    ...
    const processBlock = (block: string): void => {
      ...
      if (data.length > MAX_STREAM_EVENT_LENGTH) throw new HttpError(502, '上游流式事件过大');
      let payload: unknown;
      try {
      ...
      if (!isRecord(payload)) throw new HttpError(502, '上游流式事件格式无效');
    ...
    try {
      for (;;) {
        ...
        if (pending.length > MAX_STREAM_EVENT_LENGTH) throw new HttpError(502, '上游流式事件过大');
      }
  ...
  private async consumeJsonResponse(response: Response, state: StreamState, onDelta?: (text: string) => void): Promise<void> {
    let payload: unknown;
    try {
    ...
    if (!isRecord(payload)) throw new HttpError(502, '上游响应格式无效');
    const error = providerError(payload);
    if (error) throw new HttpError(502, error.slice(0, MAX_UPSTREAM_ERROR_LENGTH));
